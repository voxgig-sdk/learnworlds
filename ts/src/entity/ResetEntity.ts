...
class ResetEntity {
  ...
  make() {
    return new ResetEntity(this.#client, this.entopts())
  }
  ...
  toJSON() {
    return { ...(this.#data || {}), _entity: 'Reset' }
  }

  toString() {
    return 'Reset ' + this.#utility.struct.jsonify(this.#data)
  }
  ...
  async create(this: any, reqdata?: any, ctrl?: Control) {
    ...
    let op: Operation = opify({
      ...
      pathalt: [{"path":"/v2/users/{user_id}/courses/{cid}/reset","user_id":true,"cid":true}],
      params: [
  "user_id",
  "cid"
],
      alias: {},
      state: {},
      reqform: { "units": "`reqdata`" },
      resform: "`body`",
      validate: { "params": { "user_id": "`$STRING`", "cid": "`$STRING`" } },
    })
