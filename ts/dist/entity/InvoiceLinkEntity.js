...
exports.InvoiceLinkEntity = void 0;
const node_util_1 = require("node:util");
class InvoiceLinkEntity {
    #client;
    #entopts;
    #utility;
    #data;
    ...
    make() {
        return new InvoiceLinkEntity(this.#client, this.entopts());
    }
    data(data) {
        const featurehook = this.#utility.featurehook;
        if (null != data) {
    ...
    toJSON() {
        return { ...(this.#data || {}), _entity: 'InvoiceLink' };
    }
    toString() {
        return 'InvoiceLink ' + this.#utility.struct.jsonify(this.#data);
    }
    ...
    async load(reqmatch, ctrl) {
        ...
        const op = opify({
            ...
            pathalt: [{ "path": "/v2/payments/{payment_id}/invoice-link", "payment_id": true }],
            params: [
                "payment_id"
            ],
            alias: {},
            ...
            reqform: "`body`",
            resform: "`body`",
            validate: { "params": { "payment_id": "`$STRING`", "Authorization": "`$STRING`", "Lw-Client": "`$STRING`" } },
        });
        let ctx = contextify({
            current: new WeakMap(),
            ctrl,
...
exports.InvoiceLinkEntity = InvoiceLinkEntity;
