...
class CertificateEntity {
    ...
    make() {
        return new CertificateEntity(this.#client, this.entopts());
    }
    data(data) {
        const featurehook = this.#utility.featurehook;
        if (null != data) {
    ...
    toJSON() {
        return { ...(this.#data || {}), _entity: 'Certificate' };
    }
    toString() {
        return 'Certificate ' + this.#utility.struct.jsonify(this.#data);
    }
    ...
    async remove(reqmatch, ctrl) {
        ...
        let op = opify({
            ...
            name: 'remove',
            path: '/v2/certificates/{id}',
            pathalt: [{ "path": "/v2/certificates/{id}", "id": true }],
            params: [
                "id"
            ],
            alias: {},
