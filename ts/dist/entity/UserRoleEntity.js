...
class UserRoleEntity {
    ...
    make() {
        return new UserRoleEntity(this.#client, this.entopts());
    }
    data(data) {
        const featurehook = this.#utility.featurehook;
        if (null != data) {
    ...
    toJSON() {
        return { ...(this.#data || {}), _entity: 'UserRole' };
    }
    toString() {
        return 'UserRole ' + this.#utility.struct.jsonify(this.#data);
    }
    ...
    async load(reqmatch, ctrl) {
        ...
        const op = opify({
            ...
            pathalt: [{ "path": "/v2/users/{user_id}/user-role", "user_id": true }],
            params: [
                "user_id"
            ],
            alias: {},
